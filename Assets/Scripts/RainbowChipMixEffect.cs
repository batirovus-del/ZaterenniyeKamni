...
public class RainbowChipMixEffect : BombMixEffect
{
	...
	private IEnumerator MixEffect()
	{
		...
		if (base.targetId == -1)
		{
			...
			{
				if (!chip.destroing && (bool)chip.parentSlot && (!chip.parentSlot.slot || chip.parentSlot.slot.canBeCrush) && chip.id >= 0)
				{
					array2[chip.id]++;
		...
		else
		{
			...
			{
				if (!chip2.destroing && (bool)chip2.parentSlot && (!chip2.parentSlot.slot || chip2.parentSlot.slot.canBeCrush) && chip2.id == base.targetId)
				{
