...
public class RainbowBombMixEffect : BombMixEffect
{
	...
	private IEnumerator MixEffect()
	{
		...
		if (targetId == -1)
		{
			...
			{
				if (!simpleChip.destroing && (bool)simpleChip.parentSlot && (!simpleChip.parentSlot.slot || simpleChip.parentSlot.slot.canBeCrush))
				{
		...
		else
		{
			...
			{
				if (!simpleChip2.destroing && (bool)simpleChip2.parentSlot && (!simpleChip2.parentSlot.slot || simpleChip2.parentSlot.slot.canBeCrush) && simpleChip2.id == targetId)
				{
		...
		{
			if ((bool)sc && (bool)sc.chip && !sc.chip.destroing)
			{
